...
```
git clone https://github.com/XtrionNode/xtrion.git
git submodule update --init --recursive
mkdir build
cd build
-> add chmod 777 build folder
cmake -DCMAKE_BUILD_TYPE=Release ..
make -j$(nproc) xtriond
...
## Linux
1. Move to the directory where the xtriond file is located, execute the node program with the following command
   > ./xtriond
...
```
      ...
      unlocked >>> transfer test001 test002 "100.00000000 X3O" "" true
```
